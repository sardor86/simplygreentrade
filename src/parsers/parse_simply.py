...
class SimplyGreenTrade:
    ...
    BASE_URL = 'https://simplygreentrade.com'
    AUTH_URL = 'https://simplygreentrade.com/account/'
    ITEMS_PER_PAGE = 24
    ...
    def __init__(self):
        self.login = os.getenv('SIMPLY_LOGIN')
        self.password = os.getenv('SIMPLY_PASSWORD')
        self.__auth()
        self.catalog_urls = []
        self.product_urls = []
    ...
    def __auth(self):
        ...
        Authorization
        """
        response = self.session.get(self.AUTH_URL, headers=self.HEADERS)
        soup = BeautifulSoup(response.text, 'lxml')

        login_nonce = soup.find('input', {'id': 'woocommerce-login-nonce'})['value']
        data = f'username={self.login}&password={self.password}&woocommerce-login-nonce={login_nonce}&_wp_http_referer=%2Faccount%2F%3Faction%3Dlogin&login=Log+in'
    ...
    def _get_catalog_urls(self):
        ...
        soup = BeautifulSoup(response.text, 'lxml')
        self.catalog_urls = [x.find('a')['href'] for x in soup.find('ul', {'id': 'menu-desktop-horizontal-menu'}).find_all('li', class_='item-level-0')[:-1]]
    ...
    def _get_product_urls(self):
        ...
        for catalog in tqdm(self.catalog_urls):
            ...
            if product_count:
                pages_count = math.ceil(product_count / self.ITEMS_PER_PAGE)
                page_strip = 'page/'
    ...
    def _parse_details(self, url: str) -> dict | None:
        ...
        try:
            brand = soup.find('tr', class_='woocommerce-product-attributes-item--attribute_pa_brand').find('td', class_='woocommerce-product-attributes-item__value').text.strip()
        except:
            brand = name.split(' ')[0]
    ...
    def parse_catalog(self):
        ...
        with open(BASE_DIR / 'assets' / 'result.json', 'w+', encoding='utf-8') as file:
            json.dump(all_products, file, indent=4, ensure_ascii=False)


def main():
    sgt = SimplyGreenTrade()
    sgt.parse_catalog()
    # print(json.dumps(sgt._parse_details('https://simplygreentrade.com/product/monkey-king-blue-cola-smell-unbleached-rolling-papers-with-tips-24pcs-display/'), indent=4))
    # print(json.dumps(sgt._parse_details('https://simplygreentrade.com/product/puffco-travel-pack-desert-for-proxy-vaporizers/'), indent=4))
